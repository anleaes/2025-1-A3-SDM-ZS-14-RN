...
type Props = DrawerScreenProps<DrawerParamList, 'CreateVacina'>;

const CreateVacinaScreen = ({ navigation }: Props) => {
  const [nome, setNome] = useState('');
  const [descricao, setDescricao] = useState('');
  const [fabricante, setFabricante] = useState('');
  const [dosesRecomendadas, setDosesRecomendadas] = useState('');
  const API_URL = `${API_BASE_URL}vacinas/`;

  const handleSave = async () => {
    if (!nome || !fabricante || !dosesRecomendadas) {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Falha ao criar vacina.');
      Alert.alert('Sucesso', 'Vacina criada com sucesso!');
...
export default CreateVacinaScreen;